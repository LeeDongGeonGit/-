...
export default function MembershipMapPage() {
  useKakaoLoader();
    ...
    M.plugin("location").current({
      ...
      callback: function (result) {
        if (result.status === 'NS') {
          ...
          // Set to true to move map center
          } else {
  ...
  const handleRegisterClick = async () => {
    if (position !== null && selectedMarkerId !== null) {
      try {
        ...
        console.log("Updated Post Data:", updatedPrevData);
        navigate("/MembershipPage", { state: { updatedPrevData } }); // Navigate after async operation
      } catch (error) {
  ...
  return (
    <div className="PL_Main" style={{ position: "relative", width: "100%", height: "100vh" }}>
      ...
      {/* <div className="PL_searchBox">
        <input type="text" value={keyword} onChange={(e) => setKeyword(e.target.value)} placeholder="검색어를 입력하세요" className="PL_searchinput" />
